...
const ContactsOverlay = ({ isOpen, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-end bg-black bg-opacity-50 z-50">
            <div className="bg-white w-80 h-full shadow-lg p-4 overflow-y-auto transform transition-transform duration-300 ease-in-out">
                    ...
                    {contacts.length > 0 ? (
                        contacts.map((contact) => (
                            <li key={contact._id} className="flex items-center justify-between p-2 hover:bg-gray-100 rounded-md">
                                ...
                                <button
                                    onClick={() => onMessageClick(contact._id, contact.fullName)}
